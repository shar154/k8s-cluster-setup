...
describe('Egress test suite', () => {
  ...
  beforeAll(() => {
    ...
    const endpointStack = new EndpointStack(testApp, 'EndpointStack', {
      vpc: networkStack.vpc
    });
    const computeStack = new ComputeStack(testApp, 'ComputeStack', {
      vpc: networkStack.vpc
    });
  ...
  test('controlPlaneEc2', () => {
    computeStackTemplate.hasResourceProperties(
      'AWS::EC2::Instance',
      Match.objectLike({
